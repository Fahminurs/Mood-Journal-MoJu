...
try {  
    ...
    $response = [  
        ...
        'data' => array_map(function($row, $index) use ($start, $currentUserRole, $currentUserId) {  
            // Cek kepemilikan edukasi untuk admin artikel  
            $canEditDelete = false;  
            ...
            if ($currentUserRole === 'admin') {  
                ...
                try {  
                    // Query untuk memeriksa apakah edukasi milik user yang sedang login  
                    ...
                    error_log("Error checking edukasi ownership: " . $e->getMessage());  
                    $canEditDelete = false;  
                }  
            }  
            ...
            $aksiButtons = $canEditDelete   
                ? '<div class="btn-group" role="group">  
                    <button class="btn btn-sm btn-info edit-btn-edukasi" data-id="' . $row['id_artikel'] . '" title="Edit">  
                        <i class="fas fa-edit"></i>  
                    </button>  
                    <button class="btn btn-sm btn-danger delete-btn-edukasi" data-id="' . $row['id_artikel'] . '" title="Hapus">  
                        <i class="fas fa-trash"></i>  
                    </button>  
                    <button class="btn btn-sm btn-dark view-btn-edukasi" data-judul="' . htmlspecialchars($row['judul']) . '"  title="Lihat Detail">  
                        <i class="fas fa-eye"></i>  
                    </button>  
                  </div>'  
                : '<p style="font-size: 12px;">Bukan Pemilik edukasi</p>';  
